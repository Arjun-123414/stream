...
def main_app():
    ...
    # ----------------------------------
    if st.session_state.persistent_dfs:
        st.write("### Previous Large Query Results (Active Session):")
        for idx, pdf in enumerate(st.session_state.persistent_dfs):
            ...
            with st.spinner("Analyzing your query..."):
                try:
                    ...
                    if isinstance(result, dict) and "error" in result:
                        ...
                        # Check the first item to detect if there are any datetime objects
                        has_datetime = False
                        ...
                        if len(df) > 1:  # Only show download button and grid for results with more than 1 row
                            ...
                            gb.configure_default_column(filter=True, sortable=True)
                            gridOptions = gb.build()
                            AgGrid(df, gridOptions=gridOptions, height=400, width='100%')
                            num_rows = len(df)
